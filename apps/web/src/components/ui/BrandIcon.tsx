...
//
// COROS / Wahoo / iGPSPORT don't have an open vector logo, so we load their
// real favicon from Google's favicon service and fall back to a monogram if
// it can't be fetched (offline, blocked, etc.).

type BrandId = 'strava' | 'garmin' | 'coros' | 'wahoo' | 'igpsport'

type BrandMeta = {
  /** Tile background colour (brand colour for inline SVGs, neutral for favicons). */
  tile: string
...
const BRANDS: Record<BrandId, BrandMeta> = {
  strava: { tile: '#FC4C02', monogram: 'S' },
  garmin: { tile: '#007CC3', monogram: 'G' },
  coros: { tile: '#111418', domain: 'coros.com', monogram: 'C' },
  wahoo: { tile: '#0068B5', domain: 'wahoofitness.com', monogram: 'W' },
  igpsport: { tile: '#E02E26', domain: 'igpsport.com', monogram: 'I' },
}
...
function GarminMark() {
  // Simple Icons "Garmin" wordmark, recoloured white.
  return (
    <svg viewBox="0 0 24 24" width="80%" fill="#fff" aria-hidden="true">
...
function FaviconMark({ meta, name }: { meta: BrandMeta; name: string }) {
  ...
  if (failed || !meta.domain) {
    return <span>{meta.monogram}</span>
  }
  return (
    <img
