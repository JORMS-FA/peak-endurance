...
// Official brand marks for the connection sources.
//
// Strava & Garmin ship verified single-path SVG logos (from Simple Icons), so
// we embed them inline → crisp at any size, no network dependency.
//
// The rest don't have an open vector logo, so we load their real favicon from
// Google's favicon service and fall back to a monogram if it can't be fetched.

type BrandId = 'strava' | 'garmin' | 'coros' | 'wahoo' | 'igpsport' | 'samsung' | 'health'

type BrandMeta = {
  /** Tile background colour. */
  tile: string
  /** Foreground colour for the monogram fallback. */
  fg: string
  /** Domain used to fetch the favicon, when there's no inline SVG. */
  domain?: string
  /** Pixel size of the favicon inside the tile. */
  imgSize?: number
  /** Letter shown while/if the favicon can't load. */
  monogram: string
}

const BRANDS: Record<BrandId, BrandMeta> = {
  strava: { tile: '#FC4C02', fg: '#fff', monogram: 'S' },
  garmin: { tile: '#000000', fg: '#fff', monogram: 'G' },
  coros: { tile: '#FFFFFF', fg: '#111418', domain: 'coros.com', imgSize: 30, monogram: 'C' },
  wahoo: { tile: '#0068B5', fg: '#fff', domain: 'wahoofitness.com', imgSize: 22, monogram: 'W' },
  igpsport: { tile: '#FFFFFF', fg: '#111418', domain: 'igpsport.com', imgSize: 26, monogram: 'I' },
  samsung: { tile: '#FFFFFF', fg: '#111418', domain: 'samsung.com', imgSize: 26, monogram: 'S' },
  health: { tile: '#FFFFFF', fg: '#111418', domain: 'health.google', imgSize: 26, monogram: 'H' },
}

function StravaMark() {
  return (
    <svg viewBox="0 0 24 24" width="56%" height="56%" fill="#fff" aria-hidden="true">
...
function GarminMark() {
  // Simple Icons "Garmin" wordmark, recoloured white on a black tile.
  return (
    <svg viewBox="0 0 24 24" width="82%" fill="#fff" aria-hidden="true">
...
function FaviconMark({ meta, name }: { meta: BrandMeta; name: string }) {
  // Try a high-quality logo first, then the favicon service, then a monogram.
  ...
  if (idx >= sources.length) {
    return <span style={{ color: meta.fg, fontWeight: 700 }}>{meta.monogram}</span>
  }
  const s = meta.imgSize ?? 24
  return (
    <img
      src={sources[idx]}
      alt={name}
...
export function BrandIcon({ id, name, size = 44 }: { id: BrandId; name: string; size?: number }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: 'hidden',
        border: meta.tile.toUpperCase() === '#FFFFFF' ? '1px solid rgba(0,0,0,0.08)' : undefined,
      }}
      aria-label={name}
    >
      {id === 'strava' ? (
        ...
        <FaviconMark meta={meta} name={name} />
      )}
    </div>
  )
}
